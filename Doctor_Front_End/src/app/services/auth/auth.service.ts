...
export class AuthService {
  ...
  signIn(username: string, password: string): Observable<any> {
    ...
    return this.http.post(`${this.apiUrl}/users/signin`, credentials).pipe(
      tap((response: any) => {
        if (response && response.role === 'doctor') {
          this.storeTokens(response); // Store tokens if role is doctor
        } else {
          throwError(() => new Error('Access restricted to doctors only.'));
        }
      })
    );
  }
